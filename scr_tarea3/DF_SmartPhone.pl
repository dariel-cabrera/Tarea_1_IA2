...
prop(7,'Las imágenes se queden marcadas en la pantalla incluso después de haber cambiado de elementos',p).
...
prop(11,'La pantalla muestra líneas verticales y horizontales',p).
...
prop(18,'Esta activado el modo avión',p).
prop(19,'El SmartPhone no tiene cobertura',p).
prop(20,'La targeta SIM no funciona correctamente',p).
prop(21,'Está desactivada la opción "Seleccionar Red Automáticamente" en el SmartPhone',p).
prop(22,'Está desactivada la opción "Itinerancia de Datos" en el SmartPhone',p).
prop(23,'Se desactivo temporalmente el Servicio de la Operadora',p).
prop(24,'El teléfono tiene establecido un límite de consumo en los datos del plan',p).
prop(25,'La red móvil está saturada osea hay muchos usuarios conectados a ella',p).
prop(26,'Hubo un cambio de Red en el SmartPhone',p).
%Proposiciones Inferibles:
prop(16,fallo_tactil,i).
prop(17,fallo_retroiluminación,i).
...
main:-
    ...
    fwrite(W,'~2f'),
    peso_prop(27,W),reg_peso,nl,write('Se ha detectado un fallo en el Flujo de Datos Móviles del SmartPhone con peso: '),
    fwrite(W,'~2f'),
